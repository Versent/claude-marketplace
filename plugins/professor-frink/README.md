...
| **agent-os** | **Yes** | Spec-driven development with tasks.md |

## Library Scripts

Professor Frink includes helper scripts in `lib/`:

| Script | Purpose |
|--------|---------|
| `question-engine.sh` | Adaptive question batching for tiered discovery modes |
| `task-assessor.sh` | Task quality scoring and gap identification |
| `claude-folder-generator.sh` | Generate `.claude/` structure and rules |
| `principal-skinner.sh` | Supervisor with cost/duration/iteration limits |
| `progress-tracker.sh` | Rolling window progress management |
| `task-parser.sh` | Parse tasks from agent-os specs |
| `checkpoint-manager.sh` | HITL checkpoint handling |
| `session-spawner.sh` | Spawn fresh Claude sessions |
...
| Command | Description |
|---------|-------------|
| `/professor-frink:init` | Initialize Professor Frink with tiered discovery and Claude folder generation |
| `/professor-frink:onboard` | Interactive codebase onboarding for humans |
| `/professor-frink:refine` | Incrementally improve agent-os documentation |
...
## Onboarding & Enhanced Init

Professor Frink now includes comprehensive onboarding capabilities:

### Human Onboarding (`/professor-frink:onboard`)

Interactive walkthrough to help developers understand a codebase:

1. **Mission & Context** - What the project does and why
2. **Tech Stack Overview** - Technologies and their rationale
3. **Architecture Walk-through** - Components with "go deeper" options
4. **Developer Workflow** - Local setup, testing, deployment
5. **Code Navigation** - Following dependency flow through the codebase

### Enhanced Init (`/professor-frink:init`)

The init command now includes:

**Tiered Discovery Modes:**
- **Quick (10 questions)** - For simple projects or detailed existing docs
- **Standard (25 questions)** - Balanced coverage (recommended)
- **Comprehensive (45 questions)** - Deep dive for complex projects

**Task Quality Assessment:**
- Scores each task on completeness (0-100)
- Identifies missing acceptance criteria, tech details, tests
- Focuses questions on low-scoring areas

**Claude Folder Generation:**
- Generates `claude.md` (60-75 line project overview)
- Creates `.claude/rules/` with domain-based guidelines
- Suggests MCP servers based on tech stack
- Generates MCP skill files

### Documentation Refinement (`/professor-frink:refine`)

Incremental updates after task completion:
- Captures learnings from implementation
- Updates specs based on what actually worked
...
├── task-assessment.json    # Task quality scores (NEW)
├── question-state.json     # Discovery question state (NEW)
├── generator-config.json   # Claude folder generation config (NEW)
...
### .claude/ (Generated by Init)

```
.claude/
├── rules/                  # Domain-based rule files
│   ├── code-style.md       # Naming, formatting, patterns
│   ├── testing.md          # Test approach, coverage
│   ├── security.md         # Auth, validation, secrets
│   ├── api.md              # REST, responses, versioning
│   ├── frontend.md         # Components, state, styling
│   └── infrastructure.md   # IaC, environments, deployment
└── skills/                 # MCP skill files
    ├── context7/SKILL.md
    └── [other-mcp]/SKILL.md
```
...
### Onboarder Agent (`frink-onboarder`)

Guides human onboarding:
1. Presents project mission and context
2. Explains tech stack choices
3. Walks through architecture with adaptive depth
4. Demonstrates developer workflow
5. Navigates code following dependency flow

### Assessor Agent (`frink-assessor`)

Evaluates task quality:
1. Parses tasks from agent-os specs
2. Scores completeness (0-100)
3. Identifies missing criteria
4. Generates improvement suggestions
5. Focuses discovery questions on gaps
